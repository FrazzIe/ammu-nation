...
local function drawSprite(textureDict, textureName, x, y, width, height, color)
    DrawSprite(textureDict, textureName, x, y, width, height, 0.0, color.r, color.g, color.b, color.a)
end
...
local function drawTitle()
    if menus[currentMenu] then
        ...
        if menus[currentMenu].sprite then
            if type(menus[currentMenu].sprite) == "table" then
                RequestStreamedTextureDict(menus[currentMenu].sprite[1], true)
                drawSprite(menus[currentMenu].sprite[1], menus[currentMenu].sprite[2], x, y, menuWidth, titleHeight, {r = 255, g = 255, b = 255, a = 255})
            else
                RequestStreamedTextureDict(menus[currentMenu].sprite, true)
                drawSprite(menus[currentMenu].sprite, menus[currentMenu].sprite, x, y, menuWidth, titleHeight, {r = 255, g = 255, b = 255, a = 255})
            end
        else
...
function WarMenu.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        ...
        setMenuProperty(id, 'sprite', menus[parent].sprite)
...
function WarMenu.SetSpriteTitle(id, dicts)
    setMenuProperty(id, 'sprite', dicts)
end
